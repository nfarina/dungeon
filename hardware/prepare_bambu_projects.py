...
import json, subprocess, zipfile, xml.etree.ElementTree as ET, sys, re
from pathlib import Path

ROOT=Path(sys.argv[1]).resolve()
SOURCES=json.loads((ROOT/'print-plan.json').read_text())
...
for source in SOURCES:
    ...
    cmd=[BIN,'--datadir','/private/tmp/dcc-latest-hardware-bambu','--load-settings',
...
for source in SOURCES:
    ...
    config.update(filament_colour=SOURCES[source].get('palette',['#FF7C24','#FFFFFF']),filament_self_index=['1','2'],
        flush_volumes_matrix=['0','600','280','0'],flush_volumes_vector=['140']*4,
        wipe_tower_x=[str(SOURCES[source].get('tower_xy',[115,95])[0])],wipe_tower_y=[str(SOURCES[source].get('tower_xy',[115,95])[1])])
    if SOURCES[source]['colors']==1:
        for key in filament_keys:
            if isinstance(config.get(key),list) and len(config[key])==2:
                config[key]=config[key][:1]
        config.update(filament_colour=SOURCES[source].get('palette',['#FF7C24'])[:1],filament_self_index=['1'],
    ...
    for obj in tree.findall('object'):
        ...
        if len(parts)==2:
            parts[1].find("metadata[@key='name']").set('value','Numerals')
            ET.SubElement(parts[1],'metadata',key='extruder',value='2')
            colored+=1
    assert colored==(0 if SOURCES[source]['colors']==1 else 1)
    ...
    command=[BIN,'--datadir','/private/tmp/dcc-latest-hardware-bambu','--orient','0',
    ...
    with zipfile.ZipFile(ROOT/(source+'.3mf')) as z:
        ...
        assert len(saved['filament_colour'])==(1 if SOURCES[source]['colors']==1 else 2)
        objects=ET.fromstring(z.read('Metadata/model_settings.config'))
        assert len(objects.findall(".//part/metadata[@key='extruder'][@value='2']"))==(0 if SOURCES[source]['colors']==1 else 1)
    ...
    assert len(plate['filaments'])==(1 if SOURCES[source]['colors']==1 else 2),plate
    ...
    gcode=next(sliced.glob('*.gcode')).read_text()
    selections=[int(t) for t in re.findall(r'^[ \t]*T([01])(?:\s|;|$)',gcode,re.M)]
    changes=sum(a!=b for a,b in zip(selections,selections[1:]))
    reports[source].update(filament_selection_sequence=selections,filament_changes=changes)
