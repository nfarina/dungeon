...
V2 prototype: bidirectional rounded spring detent, 0–40 rounds, three parts.
"""
...
export(coupon_rack,'test-click-rack',True,True)
export(coupon_catch,'test-click-standard',True)
coupon_light=copy(light_ceiling,'light click test')
boolean(coupon_light,box('coupon bounds',10,36.5,0,26,-1,32),'INTERSECT')
export(coupon_light,'test-click-light',True)
...
for variant,source in (('standard',ceiling),('light',light_ceiling)):
    ...
    for step in range(65):
        ...
        delta=max(0,24.5+math.sqrt(max(0,(1.1+.65+.04)**2-dz**2))-25.3)
        if variant=='standard':displacements.append(round(delta,5))
        c=copy(source,'cam travel')
        for v in c.data.vertices:
            x,z=v.co.x,v.co.z
            if 24.64<=x<=31.66 and 10.29<=z<27.9:
                if x<27.8 or z<=12.65:
                    factor=1.0
                else:
                    t=max(0,min(1,(27.9-z)/(27.9-12.65)))
                    factor=t*t*(3-t)/2
                v.co.x+=delta*factor
        shift(c,z=FLOOR+PITCH+offset)
        travel.append(intersection(c,frame));bpy.data.objects.remove(c,do_unlink=True)
    all_travel[variant]=travel
    assert max(travel)<.001,(variant,travel)
REPORT['flexed_travel_intersections_mm3']=all_travel
REPORT['approximate_follower_travel_mm']=displacements
...
plate('01-test-and-reusable-parts.3mf',[('base',10,6),('test-frame-0-to-4',10,62),('ceiling-click-standard',10,132)])
plate('02-full-frame.3mf',[('frame-0-to-40',40,3)])
plate('03-base-and-ceiling.3mf',[('base',20,20),('ceiling-click-standard',20,80)])
plate('00-click-feel-test.3mf',[('test-click-rack',25,40),('test-click-standard',55,40),('test-click-light',90,40)])
