...
GUIDE=[(7.5,1.5),(17.5,1.5),(17.5,2.9),(15.5,4.9),
       (15.5,5.45),(9.5,5.45),(9.5,4.9),(7.5,2.9)]
def slider(name,soft=False):
    ob=prism_xz(name,[(7.5,1.5),(14.8,1.5),(14.8,2.9),(13.45,4.25),
        (13.45,5.45),(9.5,5.45),(9.5,4.9),(7.5,2.9)],-16.5,6)
    # Full-width rigid bearings both before and after the free spring constrain
    # lateral motion to +/-0.30 mm. The spring is NOT a bearing surface.
    boolean(ob,prism_xz('rear bearing',GUIDE,-16.5,-13.5))
    boolean(ob,prism_xz('front bearing',GUIDE,3.0,6))
    width=1.05 if soft else 1.20
    boolean(ob,box('free spring',17.45-width,17.45,-13.9,.45,1.5,2.9))
    boolean(ob,cylinder('rounded pawl',17.55,0,1.5,2.9,.95))
    # Rounded root reinforcement at the transition from bearing to beam.
    boolean(ob,cylinder('spring root',17.45-width+.2,-13.5,1.5,2.9,.45))
    boolean(ob,prism_xz('thumb',[(9.65,5.15),(14.8,5.15),(15.4,5.75),
        (15.4,9.0),(6.2,9.0),(6.2,8.95),(9.65,5.5)],-1.8,1.8))
    # One rib = standard; two ribs = light. Geometry changes spring thickness only.
    for yy in ((0,) if not soft else (-.65,.65)):
...
def stop():
    ...
    # per side at the shoulder. Printed fit must be checked; no glue assumed.
...
gentle=slider('light slider',True)
end=stop()
...
# Nominal rest fits across the full rail and both lateral extremes.
rests=[]
for original in (standard,gentle):
    for i in range(13):
        for x in (-.299,0,.299):
            for z in (-.299,0):
                c=copy(original,'rest'); translate(c,x=x,y=FIRST+i*PITCH,z=z)
...
# Worst-case leftward play still leaves 0.40 mm of pawl interference with
# the wall between pockets. The pawl lies between the two bearings, so yaw
# also cannot move it beyond their lateral envelope. Tests below use translation;
# this is geometric capture, not a force calculation.
crests=[]
for original in (standard,gentle):
    for x in (-.299,0,.299):
...
REPORT['minimum_nominal_pawl_engagement_mm']=.4
REPORT['bearing_total_lateral_clearance_mm']=.6

# Sweep a conservatively fully retracted beam through a click at lateral
# extremes. Rigid bearings stay unchanged. Approximate cantilever shape only;
# the print is needed to establish force, fatigue and actual tolerances.
travel=[]
for original in (standard,gentle):
    for x in (-.299,0,.299):
        d=.72+x
...
def make_3mf(filename,placements):
    ...
    model='<?xml version="1.0" encoding="UTF-8"?><model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02"><metadata name="Title">Cooldown rail V2</metadata><resources>'+''.join(resources)+'</resources><build>'+''.join(build)+'</build></model>'
...
make_3mf('01-test-fit.3mf',[(('test-rail-body','test-rail-numerals'),20,20),(('slider-standard',),48,20),(('slider-light',),65,20),(('end-cap',),85,20)])
make_3mf('02-full-tracker.3mf',[(('rail-body','rail-numerals'),20,20),(('slider-standard',),48,20),(('end-cap',),65,20),(('clip-card-left-0.50mm',),48,55),(('clip-card-left-0.50mm',),48,70)])
...
show(full,dark); show(full_letters,paper); show(standard,orange); translate(standard,y=FIRST+4*PITCH)
show(end,dark)
