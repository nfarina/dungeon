...
for source in ('01-test-fit','02-full-tracker'):
    ...
    cmd=[BIN,'--datadir','/private/tmp/dcc-cooldown-v2-bambu','--load-settings',
...
for source in ('01-test-fit','02-full-tracker'):
    ...
    # Round-trip through Bambu itself to refresh project metadata and thumbnails.
    command=[BIN,'--datadir','/private/tmp/dcc-cooldown-v2-bambu','--orient','0',
    ...
    with zipfile.ZipFile(ROOT/(source+'.3mf')) as z:
        ...
        assert len(saved['filament_colour'])==2
        objects=ET.fromstring(z.read('Metadata/model_settings.config'))
        assert len(objects.findall(".//part/metadata[@key='extruder'][@value='2']"))==1
    ...
    assert len(plate['filaments'])==2,plate
