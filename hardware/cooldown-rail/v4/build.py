...
def slider(name):
    ...
    # The user selected the one-rib A slider in the physical V3 test.
    for yy in (0,):
...
full,full_letters=rail('rail',LENGTH,[str(i) for i in range(13)])
test,test_letters=rail('test rail',FIRST+4*PITCH+9,['0','1','2','10','12'])
...
REPORT['spring_widths_mm']={'one_rib_selected':1.30}
REPORT['rail_body_unchanged_from']='v3'
REPORT['letter_height_mm']=2.5
REPORT['scale']='0 to 12'
...
def make_3mf(filename,placements):
    ...
    model='<?xml version="1.0" encoding="UTF-8"?><model unit="millimeter" xml:lang="en-US" xmlns="http://schemas.microsoft.com/3dmanufacturing/core/2015/02"><metadata name="Title">Card tracker 0-12 V4</metadata><resources>'+''.join(resources)+'</resources><build>'+''.join(build)+'</build></model>'
...
make_3mf('01-track-only.3mf',[(('rail-body','rail-numerals'),45,35)])
make_3mf('02-slider-cap-clips.3mf',[(('slider-A-firm',),40,35),(('end-cap',),65,35),(('clip-card-left-0.50mm',),40,70),(('clip-card-left-0.50mm',),80,70)])
make_3mf('00-small-number-test.3mf',[(('test-rail-body','test-rail-numerals'),45,35)])
...
label('POKE',-38,68,6)
label('HP / MP / COOLDOWN',-38,51,3.2)
