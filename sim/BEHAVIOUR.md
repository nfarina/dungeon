...
The rules half adds corpses, the stairwell queue, grubs that walk a fixed 4 toward the nearest corpse and flip into the fed form of what they ate, fed janitors that walk a fixed 6 toward the nearest hero by open route (they pursue across the whole floor; Floor 1 monsters still only close the last few squares), the password door, All Hands, the boss's free Snack and Understaffed, the goose's Downed state and the Pet Biscuit in its together mode: +1 attack die for his person and Bodyguard, which the brain always accepts while the goose is up (the goose, not Lucas, takes every point of damage until he is Downed). The apart mode and the throw are not modelled.

The brain half, "cleaner" (`cleanPolicy`): on a turn with nothing to fight, no awake room monster nearby and no fed janitor within eight squares, it walks to the nearest corpse worth the action and cleans it: the goose eats a small one (small only, biscuit or not), bleach goes on a medium or large one, and Spark burns a medium or large one in line of sight when the book is ready. It never bleaches a small corpse. It squashes an adjacent grub only when that grub is one move from a medium or large corpse. It never Shoves a corpse. Patch Up is cast on anyone at 2 Health or less, and on quiet turns on anyone at 3 or less. Restructuring is read at Mind 5 on the boss or anything with 2 Health. "Runner" does none of the cleaning.

Spellbooks tick a box per cast and are mastered at five (`spellMastery`, `masteryCasts`), after which each book does one more of whatever it does (`masteryBonus: "more"`): Spark rolls a third die, Patch Up heals 3, Static reaches two squares, Mop-Up takes a second corpse next to the first. Shove and Nope are not modelled. Ethan's Spark starts Floor 2 mastered. The whole rule is worth about a point of Floor 2 win rate. The other two settings, `"cooldown"` (one round shorter) and `"free"` (one free cast a floor), were measured and rejected: both are worth seven points, because they buy casts and Spark's casts are corpse cleaning.

Movement is 2d6 plus gear, except that with no monster revealed anywhere on the board the party may walk 8 instead of the roll (`strideWhenClear`, HeroQuest First Light's rule, on by default; the roll still happens and the better number stands, so it averages 8.6). Grubs do not block it (`strideGrubsCount: false`); fed janitors do. It is available on about two move turns in three on Floor 1 and one in two on Floor 2, and it was missing from the simulator until after Floor 1 was played, which had been costing the modelled party about three and a half points of win rate there.
...
**Exploration.** Floor 1's party walks a route it knows (`explore: "route"`). Floor 2's party is `"blind"`: it does not know which room holds the password, so each round it heads for the nearest room it has not opened a door into (or has not finished), by walking distance, until someone holds the Password of the Day. From then on the goal is the Sump, with detours only into the rooms on its greed list that it has not yet seen. It does know where the Sump is: the big chamber is obvious on the board. This is the difference between "finds the password by round 9" and "by round 20" on the same map, and it is why the route brain's numbers for Floor 2 were wrong.
