...
/** Art for the reverse side: a standee mirrors its own figure; a furniture tile shows its used-state card, if it has one and it has art. */
function backArtUrl(c: Card, model: string, all: Card[]): string | null {
  if (c.type === "standee") return artUrl(c, model, all);
  const b = all.find(x => x.backOf === c.id);
  return b ? artUrl(b, model, all) : null;
}

function cardJson(c: Card, model: string, all: Card[] = cards()) {
  ...
  return {
    ...
    front: renderFront(c, a ? `/art/${a.file}` : null), back: renderBack(c, backArtUrl(c, model, all)),
...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (p === "/api/cards") return json({ decks: DECK_ORDER.map(d => ({ id: d, name: DECK_NAMES[d] })), model, models: MODELS, imageSize: IMAGE_SIZE, cards: (all => all.map(c => cardJson(c, model, all)))(cards()) });
    ...
    if (p.startsWith("/preview/")) {
      ...
      return html(`<!doctype html><meta charset="utf-8"><style>${CARD_CSS} body{margin:0;background:#666;display:flex;gap:.3in;padding:.3in;flex-wrap:wrap}</style>${renderFront(c, artUrl(c, model, all))}${renderBack(c, backArtUrl(c, model, all))}`);
    }
